const articleData = {
  ...
  'voice-ai-first-line': {
    title: 'Voice AI как первая линия клиентского сервиса',
    ...
    category: 'Voice AI', date: '2026-01-08', readTime: '4 мин',
    content: [
  ...
  'n8n-ai-crm-workflow': {
    ...
    category: 'Automation', date: '2026-01-05', readTime: '5 мин',
    content: [
...
function getSlug() {
  ...
  return articleData[path] ? path : params.get('slug');
}

function listTemplate() {
  const cards = Object.entries(articleData).map(([slug, article]) => `
...
if (root) {
  const slug = getSlug();
  root.innerHTML = slug && articleData[slug] ? articleTemplate(articleData[slug]) : listTemplate();
}
